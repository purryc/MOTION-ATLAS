...
<p><b>高度限制：</b>修正参考补偿指甲标记点在触摸时仍高于屏幕的偏移；指甲角度和接触姿态可能变化，独立物理同步未验证。它不是测得的真实指腹离屏净距。低于基线的片段显示为零，不能据此判定实际接触；本统计不受浏览器手动零点影响。Home Zone 继续仅显示位置参考。</p></section>'''
    ...
    content=re.sub(r'(src="[^\"]*report-controls.js)(?:\?[^\"]*)?"',r'\1?v=1.1.3"',content)
    scripts='<script id="naturalDwellHeightData" type="application/json">'+json.dumps(data,ensure_ascii=False,allow_nan=False)+'</script><script type="module" src="../viewer/report-dwell-height.js?v=1.1.3"></script>'
